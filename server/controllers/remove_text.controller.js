...
export const remove_text = async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (user.credits < 1) {
            return res.status(400).json({
                success: false,
                message: `Insufficient credits. You have ${user.credits} credit(s), but removing text requires at least 1 credit.`
            });
        }

        console.log(`Starting text removing for user : ${user.email}. Current credits: ${user.credits}`)
        ...
        const clipdropResponse = await fetch("https://clipdrop-api.co/remove-text/v1", {
            method: "POST",
            headers: {
        ...
        if (!clipdropResponse.ok) {
            ...
            return res.status(502).json({
                success: false,
                message: "Failed to remove text. Clipdrop API encountered an error.",
                details: errorText,
            });
        }
        ...
        // Upload processed transparent image to ImageKit
        let imageKitData;
        try {
            const base64Image = Buffer.from(imageBuffer).toString("base64");
            const originalNameBase = req.file.originalname ? req.file.originalname.split(".")[0] : "image";
            const uniqueFileName = `no-text-${Date.now()}-${originalNameBase}.png`;
        ...
        console.error("Error in removeText controller:", error);
