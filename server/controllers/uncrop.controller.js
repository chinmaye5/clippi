...
export const uncrop = async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (user.credits < 1) {
            return res.status(400).json({
                success: false,
                message: `Insufficient credits. You have ${user.credits} credit(s), but uncropping image requires at least 1 credit.`
            });
        }

        console.log(`Starting image uncropping for user : ${user.email}. Current credits: ${user.credits}`);
        ...
        clipdropForm.append("extend_up", height);
        clipdropForm.append("extend_down", height);
        clipdropForm.append("extend_right", width);
        clipdropForm.append("extend_left", width);
        ...
        if (!clipdropResponse.ok) {
            ...
            return res.status(502).json({
                success: false,
                message: "Failed to uncrop image. Clipdrop API encountered an error.",
                details: errorText,
            });
        }
        ...
        // Upload processed uncrop image to ImageKit
        let imageKitData;
        try {
            const base64Image = Buffer.from(imageBuffer).toString("base64");
            const originalNameBase = req.file.originalname ? req.file.originalname.split(".")[0] : "image";
            const uniqueFileName = `uncrop-${Date.now()}-${originalNameBase}.png`;
        ...
        console.error("Error in uncrop controller:", error);
