...
export default function RemoveText() {
    ...
    const handleProcessImage = async () => {
        ...
        try {
            ...
            const response = await api.post("/image/remove-text", formData, {
                headers: {
            ...
            if (response.data.success) {
                ...
                setErrorMsg(response.data.message || "Failed to remove text.");
            }
        } catch (error) {
            console.error(error);
            setErrorMsg(
                ...
                {imageUrl ? (
                    ...
                    <div className="border-4 border-dashed border-zinc-400 p-10 sm:p-12 text-center max-w-md bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,0.05)]">
                        ...
                        <p className="text-xs text-zinc-500 uppercase leading-relaxed">
                            Upload a source image on the left, then click process. The transparent, text-removed result will show up here.
                        </p>
                    </div>
                )}
            </div>
