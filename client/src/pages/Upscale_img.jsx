...
export default function Upscale_img() {
    ...
    const handleProcessImage = async () => {
        ...
        try {
            ...
            const response = await api.post("/image/upscale-image", formData, {
                headers: {
            ...
            if (response.data.success) {
                ...
                setErrorMsg(response.data.message || "Failed to upscale image.");
            }
        } catch (error) {
            console.error(error);
            setErrorMsg(
                ...
                {imageUrl ? (
                    ...
                    <div className="border-4 border-dashed border-zinc-400 p-10 sm:p-12 text-center max-w-md bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,0.05)]">
                        ...
                        <p className="text-xs text-zinc-500 uppercase leading-relaxed">
                            Upload a source image on the left, then click process. The upscaled result will show up here.
                        </p>
                    </div>
                )}
            </div>
